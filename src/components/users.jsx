...
const Users = () => {
    ...
    const renderPhrase = () => {
        if (users.length === 1) {
            return `${users.length} человек тусанёт с тобой`;
        }else if (users.length === 2 || users.length === 3 || users.length === 4) {
            return `${users.length} человека тусанёт с тобой`;
        }else {
        ...
        if(users.length === 0) {
            return (
                <h2>
                    <span className={getUsersClasses()}>{users.length === 0 ? 'Никто с тобой не тусанёт' : renderPhrase()}</span>
                </h2>
            )
        } else {
        return (
